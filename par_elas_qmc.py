...
# pp= 1 ... 2^m
qq = int(sys.argv[2]) # second parameter from the command line
# the first QMC points will loop from from pp-th point 
# qq = 1..5 (divide into 5 portions)

from pyQMC.lattice import PolynomialLattice
# QMC points
dim_s = 256   # number of truncated dimensions
#m = 10        # N = 2^m points. Choose N=2^m with m = 20 for reference solution
m = 11
N = 2**m
q_step = 256
...
for qmc_p in lat:
   ...
   if (cnt_p == pp):
      ...
      for qmc_q in lat:
         cnt_q = cnt_q + 1
         if ((cnt_q > (qq-1)*q_step)&(cnt_q <= q_step*qq)):
             mu = MyPsi(qmc_q)
             mu.yp = qmc_q
...
print(SN)
fsav = 'S_N'+str(N)+'_p_'+str(pp)+'_q_'+str(qq)
np.savez(fsav, myp = pp, myq = qq, myN = N, mySN = SN)
