...
# N_1^{(j)} = 2^(L-k),  j=L-k=9,...,1
# N_2^{(k)} = 2^(k-1),  k=1,...,9
#
# compute
#   SN = 2^{L-1} Q_{N^{L-k,k-1}} = sum_{j=1}^{2^{L-k}} sum_{i=1}^{2^{k-1}} calF(p(j), q(i))
...
#  
# each k will be run in parallel, see sp_qmc2.pbs 
...
filename2 = qmc_dir+"/staircase2d_spod_a2_C0.1_SPOD_2dstaircase_t0/SPOD_2dstaircase_t0_m"+str(kk-1)+".json"
...
print(SN)
fsav = 'spS_N1_'+str(N1)+'_N2_'+str(N2)+'_q_'+str(qq)
np.savez(fsav, myN1 = N1, myN2 = N2, myk = kk, myL = LL, myq = qq, mySN = SN)
